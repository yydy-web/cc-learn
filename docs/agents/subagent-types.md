...
2. **CC 保持更新**——subagent 预定义类型在最新版 CC 中开箱即用，确保 CC 已更新到最新版本
...
for dir in "${DIRS[@]}"; do
  echo "=== Reviewing $dir ==="
  # 在 CC 对话中逐个指定 subagent_type: code-reviewer 发起审查
  # 或编写 Workflow 脚本：agent({ subagent_type: 'code-reviewer', prompt: `审查 ${dir} 目录下所有文件` })
done
...
CI 中集成 code-reviewer 审查推荐通过 CC Workflow 脚本实现：在脚本中使用 `agent()` 函数并指定 `subagent_type: 'code-reviewer'`，审查变更文件并拦截 critical/high 级别的 findings。预定义类型的行为约束已内置，无需在 prompt 中重复描述审查标准。
