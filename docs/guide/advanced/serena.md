...
| -------------- | ----------------------------- | -------------------- | ------------------- |
| **定位**       | IDE 级语义操作                | 代码探索和知识图谱   | 代码审查和架构分析  |
| **核心技术**   | LSP / JetBrains               | tree-sitter + SQLite | tree-sitter + 图论  |
| **语言支持**   | 40+（LSP）/ 全部（JetBrains） | 20+                  | 30+                 |
| **重构能力**   | ✅ 符号重命名、移动、安全删除 | ❌                   | ❌                  |
| **符号编辑**   | ✅ 替换、插入、删除符号体     | ❌                   | ❌                  |
| **调试能力**   | ✅ JetBrains 专属             | ❌                   | ❌                  |
| **影响分析**   | 通过引用查找                  | ✅ 专用工具          | ✅ Blast-Radius     |
| **调用链追踪** | 通过引用递归                  | ✅ `codegraph_trace` | ✅ 执行流分析       |
| **社区检测**   | ❌                            | ❌                   | ✅ Leiden 算法      |
| **可视化**     | ❌                            | ❌                   | ✅ D3.js 交互式图谱 |
| **依赖要求**   | Python (uv)                   | Node.js (npm)        | Python (pip)        |
| **免费方案**   | LSP 后端完全免费              | 完全免费             | 完全免费            |
| **最佳场景**   | 精确重构、符号级编辑          | 日常代码探索         | PR 审查、架构分析   |
