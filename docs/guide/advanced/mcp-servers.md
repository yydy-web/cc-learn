---
title: MCP 服务器
description: 掌握 MCP 服务器的配置方法，通过 Model Context Protocol 连接外部工具和数据源，扩展 Claude Code 的能力边界，支持数据库查询、第三方 API 调用和浏览器自动化等场景
---
:::info {title="📊 页面导航"}
**适用角色与上手难度**
...
**🎯 学习产出：** 掌握 MCP 服务器的配置方法，能扩展 Claude Code 的工具能力
...
详见 [CodeGraph 代码智能](/guide/advanced/code-graph/codegraph) 教程。
...
为 Claude Code 提供跨会话的持久记忆能力，自动捕获会话中的工具使用和关键发现：
...
```json
{
  "mcpServers": {
    "claude-mem": {
      "type": "stdio",
      "command": "node",
      "args": ["~/.claude/plugins/marketplaces/thedotmack/plugin/mcp/claude-mem-mcp.js"]
    }
  }
}
```
...
:::tip
Claude-Mem 通过 `npx claude-mem install` 自动配置 MCP 服务器，通常**无需手动添加**。此处仅作参考。
:::

Claude-Mem 提供 4 个 MCP 搜索工具（`search`、`timeline`、`get_observations`），支持三层渐进式检索，约 10 倍 Token 节省。
...
详见 [Claude-Mem 持久记忆](/guide/advanced/claude-mem) 教程。
