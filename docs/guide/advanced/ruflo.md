...
describe('createLineItem', () => {
  ...
  it('数量为 0 时抛出异常', () => {
    expect(() => createLineItem({ productId: 'p1', quantity: 0, price: 99 })).toThrow('quantity must be positive');
  });
});
...
describe('applyDiscount', () => {
  ...
  it('非法折扣类型抛异常', () => {
    expect(() => applyDiscount(100, { type: 'invalid', value: 10 })).toThrow('unknown discount type');
  });
});
```
...
export function WithPermission({ resource, action, children, fallback = null }: WithPermissionProps) {
...
export function WithPermission({
  ...
  action,
  children,
  fallback = <PermissionDenied resource={resource} action={action} />,
}: WithPermissionProps) {
  // ...
}
