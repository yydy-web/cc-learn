...
const filteredOptions = computed(() => props.options.filter((o) => o.enabled));
...
function Feed() {
  ...
  const fetchMore = async () => {
    const newItems = await fetch('/api/items?offset=' + items.length).then((r) => r.json());
    ...
    <Dialog.Root>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="dialog-content">
          ...
          <Dialog.Description>此操作不可撤销，确定要继续吗？</Dialog.Description>
