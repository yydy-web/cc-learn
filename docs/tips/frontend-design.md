...
<template>
  <div class="hero" style="background: #F4F1EA; padding: 120px 0;">
    <h1 style="font-family: 'Playfair Display', serif; font-size: 56px; color: #2C1810;">手工陶艺</h1>
    <p style="color: #8B7355;">每一件都是独一无二的艺术品</p>
    <button style="background: #C1774B; color: white; padding: 16px 48px;">探索作品</button>
  </div>
</template>
```
...
/* 釉料流动 —— 签名元素，只用这一次 */
.glaze-text {
  background: linear-gradient(135deg, var(--glaze-ash) 0%, var(--glaze-celadon) 40%, var(--glaze-sky) 100%);
...
.glow-ring {
  ...
  background: radial-gradient(
    ...
    /* 中心亮 */ rgba(245, 166, 35, 0.04) 50%,
    /* 渐弱 */ transparent 70% /* 消失在黑暗中 */
  );
