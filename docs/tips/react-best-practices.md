...
**🎯 学习产出：** 掌握 React 开发最佳实践，能独立用 Claude Code 进行 TDD 组件开发、状态管理和性能优化
...
describe('UserCard', () => {
  ...
  it('无头像时显示默认占位图', () => {
    ...
    expect(screen.getByAltText('张三')).toHaveAttribute('src', expect.stringContaining('default-avatar'));
  });
});
```
...
:::details Spec-Kit React 规格编写

```
> /speckit.specify
> 为 [页面/功能名称] 编写 React 前端规格：
> - 用户故事：作为 [角色]，我希望 [行为]，以便 [目的]
> - 交互行为：[描述页面交互、表单行为、状态转换]
> - 响应式要求：[桌面端和移动端的布局差异]
> - 边界情况：[加载状态、错误状态、空状态]
> - 验收标准：[每个功能的可测试条件]
> 不要涉及技术栈选择——Spec-Kit 的 /speckit.plan 阶段会处理
```

:::

:::details Spec-Kit React 组件任务拆分

```
> /speckit.tasks
> 将 [页面名称] 的 React 技术方案拆分为任务：
> - 按依赖关系排序（Types → UI → Logic → API → Tests）
> - 每个任务包含 TDD 结构（先写测试）
> - 标记可并行的任务 [P]
> - 每个任务指定具体的文件路径
```
...
:::tip
想提升 AI 生成 React UI 的设计品质？试试 [Taste Skill](/skills/frontend/taste)——它默认使用 React + Tailwind v4 + Motion library 栈，通过三旋钮和 Pre-Flight Check 强制设计纪律。
:::
