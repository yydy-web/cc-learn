...
description: ECC 提供 63 个专业 Agent、249 个 Skill、事件驱动 Hooks 自动化和安全扫描，覆盖 12+ 语言生态，全面提升 Claude Code 开发效率
---
...
**🎯 学习产出：** 掌握 ECC 企业级代码规范，能独立搭建含 Agent、Skill、Hooks、安全扫描的完整开发增强环境
...
:::tip 功能串联
ECC 是 Claude Code 生态中的"全家桶"方案——覆盖 Agent、Skill、Hooks、Rules、安全扫描五大维度。如果你只需要某个特定能力（如安全扫描或 TDD 工作流），用 `npx ecc consult` 按需安装。配合 [Superpowers](/tips/superpowers)（工程纪律）和 [Ponytail](/tips/ponytail)（效率过滤）形成完整开发矩阵。
:::
