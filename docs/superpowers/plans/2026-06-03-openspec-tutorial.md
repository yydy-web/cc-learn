# OpenSpec 教程 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在 Claude Code 教程站点中添加 OpenSpec 规格驱动开发框架的介绍和使用教程，展示如何用结构化规格文档引导 AI 编码。

**Architecture:** 新建一个高级篇页面 `docs/guide/advanced/openspec.md`，介绍 OpenSpec 的核心理念（规格驱动开发）、三步工作流（propose → apply → archive）、CLI 命令、配置方式，并展示通过 CC-Switch 安装的方式。同时更新 4 个现有页面添加交叉引用。
...
| 操作 | 文件 | 职责 |
|------|------|------|
| 创建 | `docs/guide/advanced/openspec.md` | OpenSpec 完整教程页面 |
| 修改 | `docs/guide/advanced/_meta.json` | 侧边栏添加 openspec 条目 |
| 修改 | `docs/guide/advanced/superpowers.md` | 添加 OpenSpec 社区 schema 桥接说明 |
| 修改 | `docs/index.md` | 首页高级篇描述中加入 OpenSpec |
| 修改 | `docs/tips/best-practices.md` | 添加规格驱动开发最佳实践 |

---

### Task 1: 创建 OpenSpec 教程页面

**Files:**
- Create: `docs/guide/advanced/openspec.md`

- [ ] **Step 1: 创建完整的 OpenSpec 教程页面**

Write the following file to `docs/guide/advanced/openspec.md`:
...
```bash
git add docs/guide/advanced/openspec.md
git commit -m "docs: add OpenSpec spec-driven development tutorial page"
```
...
openspec 放在 gstack 之后，因为它关注需求规格化，是更细化的工具。

- [ ] **Step 2: Commit**

```bash
git add docs/guide/advanced/_meta.json
git commit -m "docs: add OpenSpec to advanced sidebar navigation"
```

---

### Task 3: 更新 superpowers.md 添加 OpenSpec 桥接说明

**Files:**
- Modify: `docs/guide/advanced/superpowers.md`

- [ ] **Step 1: 在"与 Gstack 的对比"部分末尾添加 OpenSpec 桥接说明**

在"推荐的组合方式"行之后、`## 与 CC-Switch 配合使用` 之前，添加：
...
git commit -m "docs: add OpenSpec bridge note to Superpowers page"
```
...
git commit -m "docs: add OpenSpec to homepage features description"
```

---

### Task 5: 更新最佳实践添加规格驱动开发

**Files:**
- Modify: `docs/tips/best-practices.md`

- [ ] **Step 1: 在 Gstack 小节之后添加 OpenSpec 小节**

在"虚拟工程团队模式（Gstack）"小节之后、"## 省钱技巧"之前添加：
...
git commit -m "docs: add OpenSpec spec-driven workflow to best practices"
```
...
Run: `npm run build`
Expected: 所有页面构建成功，无报错。`doc_build/guide/advanced/openspec.html` 应出现在输出中。

- [ ] **Step 2: 检查生成的页面**

确认以下页面都正确生成：
- `doc_build/guide/advanced/openspec.html`
- `doc_build/guide/advanced/superpowers.html`（含新桥接说明）
