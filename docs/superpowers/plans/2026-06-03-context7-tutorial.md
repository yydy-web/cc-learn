# Context7 实时文档 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在 Claude Code 教程站点中添加 Context7 实时文档平台的介绍和使用教程，展示如何让 Claude Code 获取最新的库文档而不是依赖过时的训练数据。

**Architecture:** 新建一个高级篇页面 `docs/guide/advanced/context7.md`，介绍 Context7 的核心理念（实时文档注入）、两种使用模式（MCP 和 CLI+Skills）、安装配置方式，并展示通过 CC-Switch 管理 MCP 服务器的方式。同时更新 4 个现有页面添加交叉引用。
...
| 操作 | 文件 | 职责 |
|------|------|------|
| 创建 | `docs/guide/advanced/context7.md` | Context7 完整教程页面 |
| 修改 | `docs/guide/advanced/_meta.json` | 侧边栏添加 context7 条目 |
| 修改 | `docs/guide/advanced/mcp-servers.md` | 添加 Context7 作为 MCP 服务器示例 |
| 修改 | `docs/index.md` | 首页高级篇描述中加入 Context7 |
| 修改 | `docs/tips/best-practices.md` | 添加实时文档最佳实践 |

---

### Task 1: 创建 Context7 教程页面

**Files:**
- Create: `docs/guide/advanced/context7.md`

- [ ] **Step 1: 创建完整的 Context7 教程页面**

Write the following file to `docs/guide/advanced/context7.md`:
...
```bash
git add docs/guide/advanced/context7.md
git commit -m "docs: add Context7 real-time documentation tutorial page"
```
...
改为：
```json
["hooks", "mcp-servers", "custom-skills", "cc-switch", "superpowers", "gstack", "openspec", "ralph", "codegraph", "code-review-graph", "context7", "multi-agent", "automation"]
```

context7 放在 code-review-graph 之后，因为它是一个文档工具，适合作为工具链的补充。

- [ ] **Step 2: Commit**

```bash
git add docs/guide/advanced/_meta.json
git commit -m "docs: add Context7 to advanced sidebar navigation"
```

---

### Task 3: 更新 mcp-servers.md 添加 Context7 示例

**Files:**
- Modify: `docs/guide/advanced/mcp-servers.md`

- [ ] **Step 1: 在 CodeGraph 之后添加 Context7 示例**

在 `### CodeGraph 代码智能` 小节之后、`## 自建 MCP 服务器` 之前，添加：
...
git commit -m "docs: add Context7 example to MCP servers page"
```
...
git commit -m "docs: add Context7 to homepage features description"
```
...
- [ ] **Step 1: 在代码审查图谱小节之后添加 Context7 小节**

在"代码审查图谱（Code Review Graph）"小节之后、"## 省钱技巧"之前添加：
...
git commit -m "docs: add Context7 real-time docs to best practices"
```
...
Run: `npm run build`
Expected: 所有页面构建成功，无报错。`doc_build/guide/advanced/context7.html` 应出现在输出中。

- [ ] **Step 2: 检查生成的页面**

确认以下页面都正确生成：
- `doc_build/guide/advanced/context7.html`
